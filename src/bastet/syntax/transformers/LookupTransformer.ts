...
export class LookupTransformer {
    ...
    public static buildGrapicPixelLookup(
        actor: Identifier,
        resourceDefs: TransformerResult,
        filePath: string,
        activeDeclarationScope: ScopeTypeInformation
    ): MethodDefinition {
        ...
        for (let child of resourceDefs.node.children) {
            ...
            if (fileName.endsWith('.png') || fileName.endsWith('.svg')) {
                ...
                let varWithDataLoc = new VariableWithDataLocation(
                    new TypedDataLocation('ident', StringType.instance().typeId)
                );
        ...
        return new MethodDefinition(methodIdent, paramDeclList, stmtList, resultDecl, true);
    }

    public static buildIndexByIdLookup(
        actor: Identifier,
        resourceDefs: TransformerResult,
        filePath: string,
        _activeDeclarationScope: ScopeTypeInformation
    ): MethodDefinition {
        //FIXME check if map already exists for actor
        let actorResources: Map<string, Buffer> = new Map();
        this._data.set(actor, actorResources);
        ...
        const resultVarDecl = new VariableWithDataLocation(
            new TypedDataLocation('result', IntegerType.instance().typeId)
        );
        ...
        for (let child of resourceDefs.node.children) {
            ...
            if (fileName.endsWith('.png') || fileName.endsWith('.svg')) {
                ...
                let varWithDataLoc = new VariableWithDataLocation(
                    new TypedDataLocation('id', StringType.instance().typeId)
                );
                let cond = new StrEqualsExpression(varWithDataLoc, new StringLiteral(name));
    ...
    public static buildIdByIndexLookup(
        actor: Identifier,
        resourceDefs: TransformerResult,
        filePath: string,
        activeDeclarationScope: ScopeTypeInformation
        ...
        for (let child of resourceDefs.node.children) {
            ...
            if (fileName.endsWith('.png') || fileName.endsWith('.svg')) {
                ...
                let varWithDataLoc = new VariableWithDataLocation(
                    new TypedDataLocation('idx', StringType.instance().typeId)
                );
    ...
    public static buildGetImageWidthLookup(
        actor: Identifier,
        resourceDefs: TransformerResult,
        filePath: string,
        _activeDeclarationScope: ScopeTypeInformation
        ...
        let resultVarDecl = new VariableWithDataLocation(
            new TypedDataLocation('result', IntegerType.instance().typeId)
        );
        //   let declareStackVar = new DeclareStackVariableStatement(resultVarDecl);
        let stmts = [];
    ...
    public static buildGetImageHeightLookup(
        actor: Identifier,
        resourceDefs: TransformerResult,
        filePath: string,
        activeDeclarationScope: ScopeTypeInformation
    ...
    public static buildGetNumGraphics(
        actor: Identifier,
        resourceDefs: TransformerResult,
        filePath: string
    ): MethodDefinition {
